...
public class PublicKey {
    ...
    public ResponseEntity<cloud.fogbow.ms.api.http.response.PublicKey> getPublicKey() throws FogbowException {
        try {
            LOGGER.info(Messages.Log.GET_PUBLIC_KEY);
            String publicKeyValue = ApplicationFacade.getInstance().getPublicKey();
            cloud.fogbow.ms.api.http.response.PublicKey publicKey = new cloud.fogbow.ms.api.http.response.PublicKey(publicKeyValue);
